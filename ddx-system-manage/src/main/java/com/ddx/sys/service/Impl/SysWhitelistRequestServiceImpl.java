...
public class SysWhitelistRequestServiceImpl extends ServiceImpl<SysWhitelistRequestMapper, SysWhitelistRequest> implements ISysWhitelistRequestService {
    ...
    public Boolean initWhitelistConfig() {
        try {
            List<String> requestWhitelist = baseMapper.selectList(new QueryWrapper<SysWhitelistRequest>().lambda().eq(SysWhitelistRequest::getType,ConstantUtils.WHITELIST_TYPE_1))
            ...
            List<String> requestTimeWhitelist = baseMapper.selectList(new QueryWrapper<SysWhitelistRequest>().lambda().eq(SysWhitelistRequest::getType,ConstantUtils.WHITELIST_TYPE_2))
                    .stream().map(e -> { return e.getUrl(); }).collect(Collectors.toList());
